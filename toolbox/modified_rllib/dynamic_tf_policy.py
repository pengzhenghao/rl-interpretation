...
class DynamicTFPolicy(TFPolicy):
    """
    THIS CLASS IS MODIFIED BY PENGZHENGHAO IN ORDER TO ALLOW CONTROL OF
    ACTIVATION MAGINITUDE.
    """
    ...
    def set_weights(self, weights):
        ...
        if sum(size_list) != len(weights):
            logger.debug(
                "Detect the size of weights is not compatible! The input "
                "weights has length: {}, but the total required size is: {}"
                ".".format(len(weights), sum(size_list))
            )
            trunk = []
            start = now = 0
            num_ones = 0
    ...
    def copy(self, existing_inputs):
        ...
        if len(self._loss_inputs) + num_state_inputs != len(existing_inputs):
            raise ValueError(
                "Tensor list mismatch", self._loss_inputs, self._state_inputs,
                existing_inputs
            )
        for i, (k, v) in enumerate(self._loss_inputs):
            if v.shape.as_list() != existing_inputs[i].shape.as_list():
                raise ValueError(
                    "Tensor shape mismatch", i, k, v.shape,
