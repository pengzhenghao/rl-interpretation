"""copied from https://github.com/ray-project/ray/blob/master/rllib/contrib
/maddpg/maddpg_policy.py"""
...
class MADDPGTFPolicy(MADDPGPostprocessing, TFPolicy):
    def __init__(self, obs_space, act_space, config):
        ...
        self.config = config = dict(
            ray.rllib.contrib.maddpg.DEFAULT_CONFIG, **config
        )
        ...
        def _make_loss_inputs(placeholders):
            return [
                (ph.name.split("/")[-1].split(":")[0], ph)
                for ph in placeholders
            ]

        loss_inputs = _make_loss_inputs(
            obs_ph_n + act_ph_n + new_obs_ph_n + new_act_ph_n +
            [rew_ph, done_ph]
        )
    ...
    def gradients(self, optimizer, loss):
        if self.config["grad_norm_clipping"] is not None:
            self.gvs = {
                k: minimize_and_clip(
                    optimizer, self.losses[k], self.vars[k],
        ...
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE) as scope:
            if self.config["use_state_preprocessor"]:
                model_n = [
                    ...
                    ) for obs, obs_space, act_space in
                    zip(obs_n, obs_space_n, act_space_n)
                ]
    ...
    def _build_actor_network(
            self, obs, obs_space, act_space, hiddens, activation=None,
            scope=None
    ):
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE) as scope:
            if self.config["use_state_preprocessor"]:
