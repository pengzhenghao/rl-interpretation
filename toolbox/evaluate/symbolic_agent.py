...
class MaskSymbolicAgent(SymbolicAgentBase):
    ...
    def add_gaussian_perturbation_for_policy(self, policy, mean, std, seed=None):
        ...
        for mask_name, shape in mask_template.items():
            self.mask[mask_name] = \
                random_state.normal(loc=mean, scale=std,
                                    size=shape[1:])
        policy.set_default_mask(self.mask)
        return policy
    ...
    def mask_callback_for_policy(self, policy):
        if self.mask is not None:
            ...
            self.mask = mask_dict
            policy.set_default_mask(mask_dict)
            return policy
        else:
    ...
    def get(self):
        # Let's change it to restore the policy instead of agent.
        if self.initialized:
        ...
        # new codes
        self.policy = restore_policy_with_mask(run_name, ckpt_path, env_name)
        self.policy = self.mask_callback_for_policy(self.policy)
