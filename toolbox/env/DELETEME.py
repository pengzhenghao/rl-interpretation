...
class config:
    def __init__(
        ...
        self.thres = thres
        self.coef = coef
        self.experiment_name = '{0}_{1}_{2}_DivInit'.format(
            self.choices_n, self.thres, self.coef
        )
...
for repeat in range(config.num_repeat):
    ...
    Transition = namedtuple(
        'Transition', (
            'state', 'value', 'action', 'logproba', 'mask', 'next_state',
            'reward', 'action_next'
        )
    )
    EPS = 1e-10
    ...
    mkdir(
        ENV_NAME.split('-')[0] + '/CheckPoints_' + config.experiment_name,
        exist_ok=True
    )
    mkdir(
        ENV_NAME.split('-')[0] + '/Rwds_' + config.experiment_name,
        exist_ok=True
    )
    rwds = []
    ...
    class ActorCritic(nn.Module):
        ...
        def select_action_by_ensemble(
                self, action_mean, action_logstd, action_w,
            ...
            action_selected = torch.normal(
                action_mean_selected, action_std_selected
            )
            # print(action_selected.shape) #[1,1,6]
            if return_logproba:
                logproba = self._normal_logproba_MoG(
                    action_selected, action_mean, action_logstd,
                    prob_list.unsqueeze(-1)
                )
            return action_selected, logproba, rand, prob_list

        def select_action_by_parameter(
                self, action_mean, action_logstd, action_w,
                return_logproba=True
        ):
        ...
        def _normal_logproba_MoG(self, x, mean, logstd, prob_list):
            ...
            exp_part = (
                torch.exp(
                    -(
                        x.expand(self.choices_n, mean.size(1), mean.size(2)) -
                        mean
                    ).pow(2) / (2 * std.pow(2))
                ) / std
            )
        ...
        def get_logproba(self, states, actions):
            ...
            logproba = self._normal_logproba_MoG(
                actions.permute(1, 0, 2), action_mean, action_logstd, prob_lst
            )
            ...
            for i_epoch in range(int(args.num_epoch * batch_size /
                ...
                if USE_GPU:
                    ...
                    if config.choices_n > 1:
                        ...
                        (i_1, i_2) = list(
                            combinations(
                                [i for i in range(config.choices_n)], 2
                            )
                        )[0]
                        mean_distance = torch.mean(
                            torch.norm(
                                (
                                    minibatch_action_mean[i_1] -
                                    minibatch_action_mean[i_2]
                                ),
                                dim=1
                            )
                        )
                    ...
                    loss_entropy_cpu = torch.mean(
                        torch.exp(minibatch_newlogproba) *
                        minibatch_newlogproba
                    )
                    ...
                    if SELECTION_METHOD == "ddpg_critic":
                        ...
                        target_Q = network._forward_ddpg_critic(
                            ...
                            minibatch_action_nexts.cuda()
                        ).cpu().detach()
                        target_Q = (
                            minibatch_rewards +
                            (minibatch_masks * args.gamma * target_Q)
                        ).detach()
    ...
    torch.save(
        network.state_dict(),
        ENV_NAME.split('-')[0] +
        '/CheckPoints_{0}/checkpoint_{1}hidden_{2}repeat'.
        format(config.experiment_name, config.hid_num, repeat)
    )
    np.savetxt(
        ENV_NAME.split('-')[0] + '/Rwds_{0}/rwds_{1}hidden_{2}repeat'.
        format(config.experiment_name, config.hid_num, repeat), rwds
    )
    np.savetxt(
        ENV_NAME.split('-')[0] + '/Rwds_{0}/Eval_rwds_{1}hidden_{2}repeat'.
        format(config.experiment_name, config.hid_num, repeat),
        Eval_reward_logger
    )
